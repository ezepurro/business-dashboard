...
**Versión:** 1.2 (Arquitectura de Almacenamiento — MinIO + StorageProvider)  
**Fecha:** Julio 2026  
...
class DatasetService {
    ...
    + completeProcessing(datasetId, payload): Void
    + delete(datasetId, userId, role): Void
}
...
Backend -> MongoDB: Dataset.status = "processing", uploadedAt = now
activate MongoDB
deactivate MongoDB
...
Backend --> Frontend: HTTP 201 { dataset: status "processing" }
deactivate Backend
...
* `DatasetService` importa exclusivamente el tipo `StorageProvider` y la instancia resuelta por el *factory* — nunca el SDK de MinIO.
...
El nombre original del archivo se conserva en `datasets.filename` (MongoDB) únicamente para mostrarlo en la interfaz. La clave física (`objectKey`) se construye a partir de `companyId` + `datasetId` generado + extensión validada — nunca a partir de un valor provisto por el usuario, para evitar path traversal y colisiones.

### 6.4 Máquina de Estados del Dataset

| Estado | Significado | Transición |
| :--- | :--- | :--- |
| `uploading` | Documento creado, el archivo aún se está escribiendo en MinIO. | Estado inicial. |
| `processing` | Archivo confirmado en MinIO; FastAPI fue notificado. | Tras `StorageProvider.upload()` exitoso. |
| `ready` | El callback de FastAPI reportó éxito; existe un `Analysis`. | Callback `status: "success"`. |
| `failed` | Falló la escritura en MinIO, la notificación a FastAPI, o el procesamiento. | Cualquier error en el flujo, con `errorMessage` explicando la causa. |

Un dataset nunca debe quedar indefinidamente en `uploading`: si la escritura en MinIO falla, el documento se marca `failed` en la misma operación — nunca se abandona a medio camino.

### 6.5 Flujo de Eliminación en Cascada
...
### 6.6 Seguridad del Canal Interno (FastAPI → Express)
